...
class PublicReportRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        foreach (['qr_code_id','reporter_class_id','reporter_subject_id','reporter_staff_unit_id','related_class_id','location_id'] as $key) {
    ...
    public function rules(): array
    {
        return [
            ...
            'reporter_phone'=>['required','string','max:30','regex:/^[0-9+() .-]+$/',function(string $attribute,mixed $value,\Closure $fail):void{$digitCount=strlen(preg_replace('/\D+/','',(string)$value)??'');if($digitCount<8||$digitCount>15)$fail('Nomor HP harus berisi 8 sampai 15 digit.');}],
