...
function sendExtendRequestEmail($conn, $extendId) {
    ...
    foreach ($recipients as $r) {
        if (queueEmail($r['email'], $subject, $fullHtml, $r['nama'])) {
            error_log("[EMAIL] send-extend-request: EMAIL QUEUED TO: " . $r['email']);
            $totalQueued++;
        } else {
            error_log("[EMAIL] send-extend-request: EMAIL QUEUE FAILED TO: " . $r['email']);
        }
    }
    ...
    error_log("[EMAIL] send-extend-request: Total queued {$totalQueued}/" . count($recipients) . " for extend #{$extendId}");
