...
function sendReturnConfirmedEmail($conn, $peminjamanId) {
    ...
    foreach ($recipients as $r) {
        if (queueEmail($r['email'], $subject, $fullHtml, $r['nama'])) {
            error_log("[EMAIL] send-return-confirmed: EMAIL QUEUED TO: " . $r['email']);
            $totalQueued++;
        } else {
            error_log("[EMAIL] send-return-confirmed: EMAIL QUEUE FAILED TO: " . $r['email']);
        }
    }
    ...
    error_log("[EMAIL] send-return-confirmed: Total queued {$totalQueued}/" . count($recipients) . " for borrowing #{$peminjamanId}");
