...
RUN curl -fL --retry 5 --retry-delay 2 \
    ...
    && chmod 0444 /opt/laporin-ai/models/*.gguf /opt/laporin-ai/*.h
...
COPY docker/ai/laporin_ai_php.cpp /tmp/laporin_ai_php.cpp

RUN g++ -std=c++17 -fPIC -shared -DZEND_COMPILE_DL_LAPORIN_AI_NATIVE=1 \
        /tmp/laporin_ai_php.cpp \
        $(php-config --includes) \
        -I/opt/laporin-ai/include \
        -I/opt/laporin-ai/include/ggml \
        -L/opt/laporin-ai/lib -llaporin_ai \
        -Wl,-rpath,/opt/laporin-ai/lib \
        -o "$(php-config --extension-dir)/laporin_ai_native.so" \
    && printf "extension=laporin_ai_native.so\n" > /usr/local/etc/php/conf.d/laporin-ai-native.ini \
    && rm -f /tmp/laporin_ai_php.cpp
...
RUN chmod +x /usr/local/bin/laporin-start /usr/local/bin/laporin-worker-start \
    ...
    && chmod 0555 /opt/laporin-ai/lib/*.so* \
    && chmod 0444 /opt/laporin-ai/models/*.gguf /opt/laporin-ai/*.h \
    && chmod 0555 /usr/local/lib/php/extensions/*/laporin_ai_native.so \
