...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        ...
        $middleware->web(append: [SecurityHeaders::class, EnterpriseSecurity::class, InjectReportFormFix::class]);
        $middleware->alias(['active'=>EnsureActiveUser::class,'role'=>CheckRole::class]);
    })
    ...
    ->booted(function (): void {
        ...
        RateLimiter::for('public-reports', function (Request $request) {
            $ip = (string) ($request->ip() ?: 'unknown'); $device = (string) ($request->cookie('laporin_device_id') ?: 'unknown'); $appKey = (string) config('app.key');
            return [Limit::perMinutes(120,5)->by('public-report-ip:'.hash_hmac('sha256',$ip,$appKey)), Limit::perMinutes(120,5)->by('public-report-device:'.hash_hmac('sha256',$device,$appKey))];
        });
        RateLimiter::for('public-tracking', function (Request $request) { return Limit::perMinute(10)->by($request->ip() ?? 'unknown'); });
    })
    ->create();