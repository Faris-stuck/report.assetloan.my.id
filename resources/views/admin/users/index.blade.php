...
@section('content')
<div class="page-header"><div><span class="page-kicker">SuperAdmin</span><h1 class="page-title h2 mt-2">Manajemen Pengguna</h1><p class="page-subtitle">Tambah akun pengelola dengan empat peran resmi, status aktif, dan kata sandi kuat.</p></div></div>
<div class="laporin-card mb-4">
    ...
    <form method="POST" action="{{ route('admin.users.store') }}" class="row g-3 align-items-end">
        ...
        <div class="col-md-6 col-lg-3"><label class="form-label required" for="email">Surel</label><input id="email" name="email" value="{{ old('email') }}" type="email" class="form-control" placeholder="Contoh: nama@sekolah.sch.id" required maxlength="150" autocomplete="email"></div>
        <div class="col-md-6 col-lg-2"><label class="form-label required" for="password">Kata Sandi</label><input id="password" name="password" type="password" class="form-control" placeholder="Min 8 huruf+angka" required minlength="8" pattern="(?=.*[A-Za-z])(?=.*\d).{8,}" autocomplete="new-password"></div>
        <div class="col-md-6 col-lg-2"><label class="form-label required" for="role">Peran</label><select id="role" name="role" class="form-select" required>@foreach(\App\Models\User::ROLES as $role)<option value="{{ $role }}" @selected(old('role') === $role)>{{ str_replace('_',' ', $role) }}</option>@endforeach</select></div>
