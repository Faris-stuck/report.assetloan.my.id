...
<nav class="navbar navbar-expand-lg navbar-dark app-navbar sticky-top" aria-label="Navigasi utama LAPORIN">
    <div class="container mobile-shell">
        <a class="navbar-brand" href="{{ route('public.report') }}" aria-label="Beranda LAPORIN"><img class="brand-mark brand-logo" src="{{ asset('images/branding/logo tb.png') }}" alt="Logo SMK Taruna Bangsa" width="38" height="38"><span>LAPORIN</span></a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#mainNav" aria-controls="mainNav" aria-expanded="false" aria-label="Buka menu navigasi utama"><span class="navbar-toggler-icon"></span></button>
        <div id="mainNav" class="collapse navbar-collapse">
            ...
            <div class="d-flex flex-column flex-lg-row gap-2 align-items-lg-center ms-lg-3">
                @auth<div class="nav-user-chip text-truncate small" title="{{ $currentUser->name }} · {{ $currentUser->role }}">{{ $currentUser->name }} · {{ str_replace('_',' ', $currentUser->role) }}</div><form method="POST" action="{{ route('logout') }}">@csrf<button class="btn btn-sm btn-light w-100 w-lg-auto">Keluar</button></form>@endauth
            </div>
        </div>
    </div>
</nav>
